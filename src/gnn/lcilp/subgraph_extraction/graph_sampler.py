...
def sample_neg(
    ...
    while len(neg_edges) < num_neg_samples_per_link * len(pos_edges):
        neg_head, neg_tail, rel = (
            pos_edges[pbar.n % len(pos_edges)][0],
            pos_edges[pbar.n % len(pos_edges)][1],
...
def links2subgraphs(A, graphs, params, max_label_value=None):
    ...
    max_n_label = {
        "value": np.array([0, 0])
    }  # must be length 2 containint [distance_to_node_1, distance_to_node_2]
    subgraph_sizes = []
    enc_ratios = []
    num_pruned_nodes = []
    ...
    BYTES_PER_DATUM = (
        get_average_subgraph_size(100, list(graphs.values())[0]["pos"], A, params)
        * _multiplier
    )
    links_length = 0
    ...
    def extraction_helper(A, links, g_labels, split_env):
        ...
        with mp.Pool(
            ...
            initargs=(A, params, max_label_value),
        ) as p:
            args_ = zip(range(len(links)), links, g_labels)
            for str_id, datum in tqdm(
                p.imap(extract_save_subgraph, args_), total=len(links)
            ):
...
def extract_save_subgraph(args_):
    ...
    if max_label_value_ is not None:
        n_labels = np.array(
            [np.minimum(label, max_label_value_).tolist() for label in n_labels]
        )
...
def subgraph_extraction_labeling(
    ...
    labels, enclosing_subgraph_nodes = node_label(
